...
export const actions = {
    ...
    async actionQuadPostContestProblem(context: MainContext, _problem: IQuadProblemCreate) {
        ...
        try {
            ...
            console.log('pop');
            console.log('pu',response.data['body']);
            const problem: IQuadProblem = response.data['body'];
            const problem_uuid = problem.uuid;
            console.log('pu',problem_uuid);
            // const testfiles:IQuadProblemTestFiles = {training_data:}
            if (problem.problem_type == 'P') {
                const resp = await api.sendPickleTestFiles(token, _problem.contest_uuid, problem_uuid, _problem);
                const prob: IQuadProblem = resp.data['body'];
                console.log('created problem', prob);
                quadProblemCreateState.createdProblem = prob;
